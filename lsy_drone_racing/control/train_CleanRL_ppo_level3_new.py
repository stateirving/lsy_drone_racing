"""CleanRL-style PPO training directly on the level3 drone racing task."""
...
from lsy_drone_racing.control.ppo_level3_observation import (
    SPHERICAL_GATE_OBSTACLE_POLAR_OBSERVATION_LAYOUT,
...
LEVEL3_OBSERVATION_LAYOUT = SPHERICAL_GATE_OBSTACLE_POLAR_OBSERVATION_LAYOUT
...
class Level2RaceReward(VectorRewardWrapper):
    ...
    def _closest_obstacle_distance(observations: dict[str, Array]) -> Array:
        if observations["obstacles_pos"].shape[-2] == 0:
            return jp.full(observations["pos"].shape[0], 1e6, dtype=jp.float32)
...
class RaceObservation(VectorObservationWrapper):
    """Flatten level3 race observations into a PPO-friendly vector."""
    ...
    def __init__(
        ...
        n_history: int = 2,
        debug_obs: bool = False,
        zero_obstacle_obs: bool = False,
    ):
        """Initialize observation vector layout."""
        super().__init__(env)
        self.n_history = n_history
        self.debug_obs = debug_obs
        self.zero_obstacle_obs = zero_obstacle_obs
    ...
    def _build_layout(self) -> list[tuple[str, slice]]:
        ...
        add("ang_vel", 3)
        add("gravity_body", 3)
        add("target_gate_spherical", 4)
        add("next_gate_spherical", 4)
        add("nearest_gate1_spherical", 4)
        add("nearest_gate2_spherical", 4)
        add("nearest_obstacles_polar_xy", 2 * self.n_local_obstacles)
    ...
    def _flatten_observations(
        ...
        nearest_obstacles_polar_xy = self._nearest_obstacles_polar_xy(observations, pos, rot)
        parts = [
            pos[:, 2:3],
            vel_body,
            ang_vel,
    ...
    def _gate_spherical_target_next_nearest_observation(
        ...
        target_gate_spherical = self._gate_spherical_observation(
            observations, active_target_gate, reference_pos, reference_rot_t
        )
        next_gate = jp.minimum(active_target_gate + 1, self.n_gates - 1)
        target_gate_pos, target_gate_quat = self._select_gate(observations, active_target_gate)
        target_gate_rot_t = jp.swapaxes(self.quat_to_rotmat(target_gate_quat), -1, -2)
        next_gate_spherical = self._gate_spherical_observation(
            observations, next_gate, target_gate_pos, target_gate_rot_t
        )
        has_next_gate = active_target_gate < (self.n_gates - 1)
        next_gate_spherical = jp.where(
            has_next_gate[:, None], next_gate_spherical, jp.zeros_like(next_gate_spherical)
        )
        nearest_gate_idx = self._nearest_gate_indices_excluding_target(
            observations, reference_pos, active_target_gate
        )
        nearest_gate1_spherical = self._gate_spherical_observation(
            observations, nearest_gate_idx[:, 0], reference_pos, reference_rot_t
        )
        nearest_gate2_spherical = self._gate_spherical_observation(
            observations, nearest_gate_idx[:, 1], reference_pos, reference_rot_t
        )
    ...
    def _nearest_gate_indices_excluding_target(
        ...
        relative = observations["gates_pos"] - pos[:, None, :]
        distance = jp.linalg.norm(relative, axis=-1)
        gate_idx = jp.arange(self.n_gates)[None, :]
        masked_distance = jp.where(gate_idx == active_target_gate[:, None], jp.inf, distance)
        return jp.argsort(masked_distance, axis=-1)[:, :2]

    @staticmethod
    def _obstacle_polar_xy_by_obstacle(
        observations: dict[str, Array], pos: Array, rot: Array
    ) -> Array:
        """Return [XY distance, body-frame azimuth] per vertical obstacle."""
        ...
        theta = jp.arctan2(relative_left, relative_forward)
        return jp.stack([distance_xy, theta], axis=-1)

    def _nearest_obstacles_polar_xy(
        self, observations: dict[str, Array], pos: Array, rot: Array
    ) -> Array:
        """Return nearest vertical obstacle features sorted by XY distance."""
        if self.zero_obstacle_obs or self.n_obstacles == 0:
            return jp.zeros((pos.shape[0], 2 * self.n_local_obstacles), dtype=jp.float32)
        features = self._obstacle_polar_xy_by_obstacle(observations, pos, rot)
        n_present = features.shape[1]
        n_take = min(self.n_local_obstacles, n_present)
        nearest_idx = jp.argsort(features[..., 0], axis=-1)[:, :n_take]
        nearest = jp.take_along_axis(features, nearest_idx[..., None], axis=1)
        if n_take < self.n_local_obstacles:
            padding = jp.zeros(
                (pos.shape[0], self.n_local_obstacles - n_take, 2), dtype=jp.float32
            )
            nearest = jp.concatenate([nearest, padding], axis=1)
        return nearest.reshape(pos.shape[0], -1)

    def _select_gate(self, observations: dict[str, Array], gate_idx: Array) -> tuple[Array, Array]:
        """Return selected gate positions and quaternions for each environment."""
    ...
    def _gate_spherical_observation(
        ...
        gate_pos, gate_quat = self._select_gate(observations, gate_idx)
        gate_rot = self.quat_to_rotmat(gate_quat)
        relative_world = gate_pos - reference_pos
        relative_ref = jp.einsum("nij,nj->ni", reference_rot_t, relative_world)
        r = jp.linalg.norm(relative_ref, axis=-1)
        xy = jp.linalg.norm(relative_ref[:, :2], axis=-1)
        theta = jp.arctan2(relative_ref[:, 1], relative_ref[:, 0])
        phi = jp.arctan2(relative_ref[:, 2], xy)
        direction_world = relative_world / jp.maximum(r[:, None], 1e-6)
        gate_normal_world = gate_rot[:, :, 0]
        normal_alignment = jp.sum(gate_normal_world * direction_world, axis=-1)
        alpha = jp.arccos(jp.clip(normal_alignment, -1.0, 1.0))
        return jp.stack([r, theta, phi, alpha], axis=-1).astype(jp.float32)
...
def make_envs(
    ...
    observation_config = train_cfg.get("observation", {}) if train_cfg is not None else {}
    zero_obstacle_obs = bool(observation_config.get("zero_obstacle_obs", False))
    ...
    env = RaceObservation(
        env,
        n_history=coefs.get("n_obs", 2),
        debug_obs=debug_obs,
        zero_obstacle_obs=zero_obstacle_obs,
    )
