...
from lsy_drone_racing.control.ppo_level3_observation import (
    checkpoint_hidden_dim as level3_checkpoint_hidden_dim,
)
from lsy_drone_racing.control.ppo_level3_observation import (
    make_checkpoint as make_level3_checkpoint,
)
from lsy_drone_racing.control.ppo_level3_observation import (
    unpack_checkpoint as unpack_level3_checkpoint,
)
...
def train_wrapper_config(config: str) -> tuple[Any, dict[str, Any]]:
    """Load train-side wrapper config for the fast JAX path."""
    cfg = load_config(ROOT / "config" / config)
    return cfg, dict(cfg.get("train", {}) or {})


def seed_key(seed: int, offset: int) -> Array:
    """Create a deterministic wrapper PRNG key."""
    seed = 0 if seed is None or int(seed) == -1 else int(seed)
    return jax.random.PRNGKey(seed + offset)


def initial_thrust_state(args: JaxPPOArgs, cfg: Any, num_envs: int) -> dict[str, Array]:
    """Initialize JAX state for train.thrust."""
    _cfg, train_cfg = train_wrapper_config(args.config)
    thrust_cfg = train_cfg.get("thrust") or {}
    key = seed_key(int(args.seed), 1_003)
    key, scale_key, sag_key = jax.random.split(key, 3)
    scale = jax.random.uniform(
        scale_key,
        (num_envs,),
        dtype=jnp.float32,
        minval=float(thrust_cfg.get("scale_min", 1.0)),
        maxval=float(thrust_cfg.get("scale_max", 1.0)),
    )
    sag = jax.random.uniform(
        sag_key,
        (num_envs,),
        dtype=jnp.float32,
        minval=float(thrust_cfg.get("battery_sag_min", 0.0)),
        maxval=float(thrust_cfg.get("battery_sag_max", 0.0)),
    )
    return {
        "key": key,
        ...
        "scale": scale,
        "sag": sag,
        "horizon_steps": jnp.array(
            float(cfg.env.freq) * float(thrust_cfg.get("battery_sag_horizon_s", 10.0)),
            dtype=jnp.float32,
        ),
    }


def apply_thrust_sag(action: Array, state: dict[str, Array]) -> Array:
    """Apply train.thrust scale and linear battery sag to physical thrust."""
    progress = jnp.clip(state["steps"].astype(jnp.float32) / state["horizon_steps"], 0.0, 1.0)
    thrust_scale = jnp.maximum(state["scale"] - state["sag"] * progress, 0.0)
    return action.at[..., -1].set(action[..., -1] * thrust_scale)


def update_thrust_state(
    state: dict[str, Array], done: Array, args: JaxPPOArgs
) -> dict[str, Array]:
    """Advance train.thrust state and resample completed env slots."""
    _cfg, train_cfg = train_wrapper_config(args.config)
    thrust_cfg = train_cfg.get("thrust") or {}
    key, scale_key, sag_key = jax.random.split(state["key"], 3)
    scale = jax.random.uniform(
        scale_key,
        state["scale"].shape,
        dtype=jnp.float32,
    ...
    return {
        **state,
        "key": key,
        "steps": jnp.where(done, 0, state["steps"] + 1),
        "scale": jnp.where(done, scale, state["scale"]),
        "sag": jnp.where(done, sag, state["sag"]),
    }


def initial_action_lag_state(
    args: JaxPPOArgs, cfg: Any, action_low: Array, action_high: Array, num_envs: int
) -> dict[str, Array]:
    """Initialize JAX state for train.action_latency and train.command_response."""
    _cfg, train_cfg = train_wrapper_config(args.config)
    latency_cfg = train_cfg.get("action_latency") or {}
    response_cfg = train_cfg.get("command_response") or {}
    delay_max = int(latency_cfg.get("delay_max_steps", 0))
    neutral = jnp.broadcast_to((action_low + action_high) / 2.0, (num_envs, action_low.shape[0]))
    buffer = jnp.repeat(neutral[:, None, :], delay_max + 1, axis=1)
    key = seed_key(int(args.seed), 10_003)
    key, delay_key, rp_key, yaw_key, thrust_key = jax.random.split(key, 5)
    return {
        "key": key,
        "neutral": neutral,
        "buffer": buffer,
        ...
        "delay_steps": jax.random.randint(
            delay_key,
            (num_envs,),
            minval=int(latency_cfg.get("delay_min_steps", 0)),
            maxval=delay_max + 1,
            dtype=jnp.int32,
        ),
        "rp_tau": jax.random.uniform(
            ...
            (num_envs,),
            dtype=jnp.float32,
            minval=float(response_cfg.get("rp_tau_min_s", response_cfg.get("rpy_tau_min_s", 0.0))),
            maxval=float(response_cfg.get("rp_tau_max_s", response_cfg.get("rpy_tau_max_s", 0.0))),
        ),
        "yaw_tau": jax.random.uniform(
            yaw_key,
            (num_envs,),
            dtype=jnp.float32,
            minval=float(response_cfg.get("yaw_tau_min_s", response_cfg.get("rp_tau_min_s", 0.0))),
            maxval=float(response_cfg.get("yaw_tau_max_s", response_cfg.get("rp_tau_max_s", 0.0))),
        ),
        "thrust_tau": jax.random.uniform(
            thrust_key,
            (num_envs,),
            dtype=jnp.float32,
            minval=float(response_cfg.get("thrust_tau_min_s", 0.0)),
            maxval=float(response_cfg.get("thrust_tau_max_s", 0.0)),
        ),
        "dt": jnp.array(1.0 / float(cfg.env.freq), dtype=jnp.float32),
    }


def tau_to_alpha(tau_s: Array, dt: Array) -> Array:
...
def apply_action_latency_response(
    action: Array, state: dict[str, Array], action_low: Array, action_high: Array
) -> tuple[Array, dict[str, Array]]:
    """Apply train.action_latency and train.command_response to physical actions."""
    buffer = jnp.concatenate([action[:, None, :], state["buffer"][:, :-1, :]], axis=1)
    delayed = buffer[jnp.arange(action.shape[0]), state["delay_steps"]]
    rp_alpha = tau_to_alpha(state["rp_tau"], state["dt"])
    yaw_alpha = tau_to_alpha(state["yaw_tau"], state["dt"])
    thrust_alpha = tau_to_alpha(state["thrust_tau"], state["dt"])
    alpha = jnp.stack([rp_alpha, rp_alpha, yaw_alpha, thrust_alpha], axis=-1)
    applied = state["applied"] + alpha * (delayed - state["applied"])
    applied = jnp.clip(applied, action_low, action_high)
    return applied, {**state, "buffer": buffer, "applied": applied}


def update_action_lag_state(
    state: dict[str, Array], done: Array, args: JaxPPOArgs
) -> dict[str, Array]:
    """Reset train.action_latency/command_response state for completed env slots."""
    _cfg, train_cfg = train_wrapper_config(args.config)
    latency_cfg = train_cfg.get("action_latency") or {}
    response_cfg = train_cfg.get("command_response") or {}
    key, delay_key, rp_key, yaw_key, thrust_key = jax.random.split(state["key"], 5)
    delay_steps = jax.random.randint(
        delay_key,
        state["delay_steps"].shape,
        minval=int(latency_cfg.get("delay_min_steps", 0)),
        maxval=int(latency_cfg.get("delay_max_steps", 0)) + 1,
        dtype=jnp.int32,
    )
    rp_tau = jax.random.uniform(
        rp_key,
    ...
    thrust_tau = jax.random.uniform(
        thrust_key,
        state["thrust_tau"].shape,
        dtype=jnp.float32,
        minval=float(response_cfg.get("thrust_tau_min_s", 0.0)),
        maxval=float(response_cfg.get("thrust_tau_max_s", 0.0)),
    )
    done = done.astype(bool)
    reset_action_mask = done[:, None]
    reset_buffer_mask = done[:, None, None]
    neutral_buffer = jnp.repeat(state["neutral"][:, None, :], state["buffer"].shape[1], axis=1)
    return {
        **state,
        "key": key,
        "applied": jnp.where(reset_action_mask, state["neutral"], state["applied"]),
        "buffer": jnp.where(reset_buffer_mask, neutral_buffer, state["buffer"]),
        "delay_steps": jnp.where(done, delay_steps, state["delay_steps"]),
        "rp_tau": jnp.where(done, rp_tau, state["rp_tau"]),
        "yaw_tau": jnp.where(done, yaw_tau, state["yaw_tau"]),
        "thrust_tau": jnp.where(done, thrust_tau, state["thrust_tau"]),
    }


def repeat_obs_buffer(observations: dict[str, Array], delay_max: int) -> dict[str, Array]:
    """Create observation-delay buffers initialized with current observations."""
    return {
        key: jnp.repeat(value[:, None, ...], delay_max + 1, axis=1)
        for key, value in observations.items()
    }


def select_delayed_observations(state: dict[str, Any]) -> dict[str, Array]:
    """Select per-env delayed observations from observation buffers."""
    env_idx = jnp.arange(state["delay_steps"].shape[0])
    return {key: value[env_idx, state["delay_steps"]] for key, value in state["buffer"].items()}
...
def noise_quat(quat: Array, key: Array, std_rad: float) -> Array:
    ...
    noisy_quat = quat_multiply(delta_quat, quat)
    return noisy_quat / jnp.linalg.norm(noisy_quat, axis=-1, keepdims=True)


def add_observation_noise(
    state: dict[str, Any], observations: dict[str, Array], args: JaxPPOArgs
) -> tuple[dict[str, Array], dict[str, Any]]:
    """Apply train.observation_noise to delayed raw observations."""
    _cfg, train_cfg = train_wrapper_config(args.config)
    noise_cfg = train_cfg.get("observation_noise") or {}
    key = state["key"]
    noisy = dict(observations)
    for obs_key, std_name in (
        ("pos", "pos_std_m"),
        ("vel", "vel_std_mps"),
        ("ang_vel", "ang_vel_std_radps"),
        ("gates_pos", "gate_pos_std_m"),
        ("obstacles_pos", "obstacle_pos_std_m"),
    ):
        std = float(noise_cfg.get(std_name, 0.0))
        if std > 0.0 and obs_key in noisy:
            key, subkey = jax.random.split(key)
            noisy[obs_key] = noisy[obs_key] + std * jax.random.normal(
                subkey, noisy[obs_key].shape, dtype=noisy[obs_key].dtype
            )
    quat_std = float(noise_cfg.get("quat_rpy_std_rad", 0.0))
    if quat_std > 0.0 and "quat" in noisy:
        key, subkey = jax.random.split(key)
        noisy["quat"] = noise_quat(noisy["quat"], subkey, quat_std)
    gate_quat_std = float(noise_cfg.get("gate_rpy_std_rad", 0.0))
    if gate_quat_std > 0.0 and "gates_quat" in noisy:
        key, subkey = jax.random.split(key)
        noisy["gates_quat"] = noise_quat(noisy["gates_quat"], subkey, gate_quat_std)
    return noisy, {**state, "key": key}


def initial_observation_state(
    args: JaxPPOArgs, raw_obs: dict[str, Array]
) -> tuple[dict[str, Array], dict[str, Any]]:
    """Initialize train.observation_latency/noise state and return the policy observation."""
    _cfg, train_cfg = train_wrapper_config(args.config)
    latency_cfg = train_cfg.get("observation_latency") or {}
    delay_max = int(latency_cfg.get("delay_max_steps", 0))
    key = seed_key(int(args.seed), 20_003)
    key, delay_key = jax.random.split(key)
    state = {
        "key": key,
        "buffer": repeat_obs_buffer(raw_obs, delay_max),
        "delay_steps": jax.random.randint(
            delay_key,
            (raw_obs["pos"].shape[0],),
            minval=int(latency_cfg.get("delay_min_steps", 0)),
    ...
    delayed = select_delayed_observations(state)
    return add_observation_noise(state, delayed, args)


def apply_observation_latency_noise(
    state: dict[str, Any], raw_obs: dict[str, Array], done: Array, args: JaxPPOArgs
) -> tuple[dict[str, Array], dict[str, Any]]:
    """Apply train.observation_latency/noise and reset completed env slots."""
    _cfg, train_cfg = train_wrapper_config(args.config)
    latency_cfg = train_cfg.get("observation_latency") or {}
    delay_max = int(latency_cfg.get("delay_max_steps", 0))
    buffer = {
        key: jnp.concatenate([value[:, None, ...], state["buffer"][key][:, :-1, ...]], axis=1)
        for key, value in raw_obs.items()
    }
    working_state = {**state, "buffer": buffer}
    delayed = select_delayed_observations(working_state)
    noisy, working_state = add_observation_noise(working_state, delayed, args)
    fresh_buffer = repeat_obs_buffer(raw_obs, delay_max)
    done = done.astype(bool)
    reset_buffer = {
        key: jnp.where(
            done.reshape((done.shape[0], 1) + (1,) * (value.ndim - 2)),
            fresh_buffer[key],
            value,
        )
        for key, value in working_state["buffer"].items()
    }
    key, delay_key = jax.random.split(working_state["key"])
    delay_steps = jax.random.randint(
        delay_key,
        working_state["delay_steps"].shape,
        minval=int(latency_cfg.get("delay_min_steps", 0)),
        maxval=delay_max + 1,
        dtype=jnp.int32,
    )
    return noisy, {
        **working_state,
        "key": key,
        "buffer": reset_buffer,
        "delay_steps": jnp.where(done, delay_steps, working_state["delay_steps"]),
    }


def torch_state_dict_from_fast_params(params: dict[str, Any]) -> dict[str, Any]:
    """Convert fast JAX params to the PyTorch state_dict layout used by inference."""
    import torch
    ...
    return {
        "critic.0.weight": tensor(params["critic"]["w1"].T),
        "critic.0.bias": tensor(params["critic"]["b1"]),
        "critic.2.weight": tensor(params["critic"]["w2"].T),
        "critic.2.bias": tensor(params["critic"]["b2"]),
        "critic.4.weight": tensor(params["critic"]["w3"].T),
        "critic.4.bias": tensor(params["critic"]["b3"]),
        "actor_mean.0.weight": tensor(params["actor"]["w1"].T),
        "actor_mean.0.bias": tensor(params["actor"]["b1"]),
        "actor_mean.2.weight": tensor(params["actor"]["w2"].T),
        "actor_mean.2.bias": tensor(params["actor"]["b2"]),
        "actor_mean.4.weight": tensor(params["actor"]["w3"].T),
        "actor_mean.4.bias": tensor(params["actor"]["b3"]),
        "actor_logstd": tensor(params["actor"]["log_std"][None, :]),
    }


def fast_params_from_torch_state_dict(
    state_dict: dict[str, Any], device: jax.Device
) -> dict[str, Any]:
    """Convert a PyTorch inference state_dict to the fast JAX param layout."""
...
def load_level3_torch_initial_params(
    path: Path,
    *,
    device: jax.Device,
    expected_layout: str,
    ...
    """Load Level3 PyTorch inference weights for a warm-start with fresh optimizer state."""
    import torch

    checkpoint = torch.load(path, map_location="cpu")
    state_dict, observation_layout = unpack_level3_checkpoint(checkpoint)
    if observation_layout != expected_layout:
        raise ValueError(
            f"Initial checkpoint layout {observation_layout!r} != expected {expected_layout!r}."
        )
    hidden_dim = level3_checkpoint_hidden_dim(checkpoint, state_dict)
    if hidden_dim != expected_hidden_dim:
        raise ValueError(
            f"Initial checkpoint hidden_dim={hidden_dim} != expected {expected_hidden_dim}."
        )
    obs_dim = int(state_dict["actor_mean.0.weight"].shape[1])
    action_dim = int(state_dict["actor_mean.4.bias"].shape[0])
    if obs_dim != expected_obs_dim:
        raise ValueError(f"Initial checkpoint obs_dim={obs_dim} != expected {expected_obs_dim}.")
    if action_dim != expected_action_dim:
        raise ValueError(
            f"Initial checkpoint action_dim={action_dim} != expected {expected_action_dim}."
        )
    return fast_params_from_torch_state_dict(state_dict, device)


def save_level3_torch_checkpoint(params: dict[str, Any], path: Path, hidden_dim: int) -> None:
    """Save fast JAX Level3 params in the existing PyTorch inference checkpoint format."""
    import torch

    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        make_level3_checkpoint(
            torch_state_dict_from_fast_params(params),
...
def nearest_obstacle_heading_xy(observations: dict[str, Array], pos: Array, rot: Array) -> Array:
    """Return two nearest Level3 obstacle slots, padding with zeros when fewer exist."""
    n_local_obstacles = 2
    n_obstacles = observations["obstacles_pos"].shape[1]
    if n_obstacles == 0:
        return jnp.zeros((pos.shape[0], 4 * n_local_obstacles), dtype=jnp.float32)
    features = obstacle_heading_xy_by_obstacle(observations, pos, rot)
    n_take = min(n_local_obstacles, n_obstacles)
    nearest_obstacle_idx = jnp.argsort(features[..., 2], axis=-1)[:, :n_take]
    nearest = jnp.take_along_axis(features, nearest_obstacle_idx[..., None], axis=1)
    if n_take < n_local_obstacles:
        padding = jnp.zeros(
            (pos.shape[0], n_local_obstacles - n_take, features.shape[-1]), dtype=features.dtype
        )
        nearest = jnp.concatenate([nearest, padding], axis=1)
...
def flatten_obs_level3(observations: dict[str, Array], history: Array, last_action: Array) -> Array:
    ...
    nearest_obstacles = nearest_obstacle_heading_xy(observations, pos, rot)
    return jnp.concatenate(
        [
            pos[:, 2:3],
            vel_body,
...
def closest_obstacle_distance(observations: dict[str, Array]) -> Array:
    ...
    if observations["obstacles_pos"].shape[1] == 0:
        return jnp.full((observations["pos"].shape[0],), 1e6, dtype=jnp.float32)
...
def reward_components(
    ...
    speed = jnp.linalg.norm(observations["vel"], axis=-1)
    speed_excess = jnp.maximum(0.0, speed - float(coefs.get("speed_limit_mps", 0.0))) ** 2
    ...
    components = {
        ...
        * wrong_side_gate.astype(jnp.float32),
        ...
        "speed_excess": -float(coefs.get("speed_excess_coef", 0.0)) * speed_excess,
...
def make_initial_state(
    ...
    cfg, _train_cfg = train_wrapper_config(args.config)
    kind = observation_kind(args.level)
    policy_obs, observation_state = initial_observation_state(args, raw_obs)
    history_row = basic_history(kind, policy_obs)
    history = jnp.repeat(history_row[:, None, :], int(args.n_obs), axis=1)
    last_action = jnp.zeros((raw_obs["pos"].shape[0], 4), dtype=jnp.float32)
    obs = flatten_observation(kind, policy_obs, history, last_action)
    ...
    return FastState(
        ...
        info={
            ...
            "action_lag_state": initial_action_lag_state(
                args, cfg, action_low, action_high, raw_obs["pos"].shape[0]
            ),
            "thrust_state": initial_thrust_state(args, cfg, raw_obs["pos"].shape[0]),
...
def build_fast_env_step(
    ...
    *,
    ...
    def env_step(
        ...
        sim_action, action_lag_state = apply_action_latency_response(
            sim_action, state.info["action_lag_state"], action_low, action_high
        )
        sim_action = apply_thrust_sag(sim_action, state.info["thrust_state"])
        ...
        action_lag_state = update_action_lag_state(action_lag_state, done, args)
        thrust_state = update_thrust_state(state.info["thrust_state"], done, args)
        policy_obs, observation_state = apply_observation_latency_noise(
            state.info["observation_state"], raw_obs, done, args
        )
        obs = flatten_observation(kind, policy_obs, state.info["history"], action_norm)
        history_row = basic_history(kind, policy_obs)
        ...
        next_state = state._replace(
            ...
            info={
                ...
                "action_lag_state": action_lag_state,
        ...
        if per_env_metrics:
            metrics |= {f"reward_{name}": value for name, value in components.items()}
            metrics |= {f"race_{name}": value for name, value in race_metrics.items()}
        else:
...
def train_ppo_fast(
    ...
    *,
    ...
    if resume_from is not None and initial_model_path is not None:
        raise ValueError("Use either resume_from or initial_model_path, not both.")
    ...
    print(
        {
            ...
            "note": (
                "train-side thrust/action/observation wrappers are reimplemented "
                "as JAX state inside the scanned rollout"
            ),
        }
    )
        ...
        try:
            ...
            if resume_from is not None:
                ...
                if args.level != "level3":
                    raise ValueError(
                        "initial_model_path warm-start is currently implemented for level3."
                    )
                params = load_level3_torch_initial_params(
                    Path(initial_model_path),
                    device=device,
                    expected_layout=layout,
                    expected_obs_dim=obs_dim,
                    expected_action_dim=action_dim,
                    expected_hidden_dim=int(args.hidden_dim),
                )
                opt_state = optimizer.init(params)
                print(
                    f"warm-started fast JAX weights from {initial_model_path}; "
            ...
            for iteration in range(1, args.num_iterations + 1):
                ...
                while next_checkpoint_step is not None and global_step >= next_checkpoint_step:
                    ...
                    if args.level == "level3":
                        torch_checkpoint_path = checkpoint_path.with_suffix(".ckpt")
                        save_level3_torch_checkpoint(
                            params, torch_checkpoint_path, int(args.hidden_dim)
                        )
                        print(f"inference checkpoint saved to {torch_checkpoint_path}")
            ...
            if model_path is not None:
                ...
                if args.level == "level3":
                    torch_model_path = model_path.with_suffix(".ckpt")
                    save_level3_torch_checkpoint(params, torch_model_path, int(args.hidden_dim))
                    print(f"inference checkpoint saved to {torch_model_path}")
