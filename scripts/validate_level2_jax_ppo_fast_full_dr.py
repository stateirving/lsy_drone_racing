"""Train and evaluate the Level2 PPO preset with fast JAX full train DR."""
...
DEFAULT_RUN_NAME = "jax_fast_full_dr_level2_fast2_validation"
...
def main() -> None:
    """Run the fast full-DR validation train/eval workflow."""
    args = parse_args()
    ...
    if summary["success_rate"] < args.success_threshold:
        raise SystemExit(
            f"Fast full-DR Level2 JAX PPO validation failed: "
            f"success_rate={summary['success_rate']:.2%} "
            f"< threshold={args.success_threshold:.2%}."
        )
    print(
        f"Fast full-DR Level2 JAX PPO validation passed: "
        f"success_rate={summary['success_rate']:.2%} >= {args.success_threshold:.2%}"
    )
